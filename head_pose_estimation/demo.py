    ...
    with torch.no_grad():
        while True:
            ...
            for box, landmarks, score in faces:
                ...
                result = str(p_pred_deg.item())+(" , ")+str(y_pred_deg.item())+(" , ")+str(r_pred_deg.item())+(" , ")+str(x_min + int(.5*( x_max-x_min)))+(" , ")+str(y_min + int(.5*(y_max-y_min)))
                print(result)
                client.sendto(result.encode('utf-8'),(HOST,PORT))

            cv2.imshow("Demo", frame)
            client.sendto("0".encode('utf-8'),(HOST,PORT))
            cv2.waitKey(5)
